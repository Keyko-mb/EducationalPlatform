...
import {useFilesStore} from "@/stores/files.js";
  ...
  if (answer.value) answerText.value = answer.value.text || ''
} catch (error) {
  ...
  answerFiles.value = answerFiles.value.filter(f => f.name !== file.name)
    ...
    <div>
      ...
      <div v-else-if="answer && answer.statusId === 1">
        <form @submit.prevent="submitAnswer" class="flex flex-col">
          ...
          <textarea id="answer-text" v-model="answerText" class="my-input h-28 w-full" placeholder="Начните писать ответ..." :aria-label="answerText"></textarea>
          ...
          <div v-if="answerFiles.length > 0">
            ...
            <div class="border border-tertiary rounded-lg max-h-96 flex-wrap overflow-y-auto bg-formColor p-4 flex gap-4" role="list">
              <div v-for="(file, index) in answerFiles" :key="'file-' + index" class="flex items-center mb-2" role="listitem">
                <div v-if="file.type.startsWith('image')" class="relative">
                  ...
                  <button @click.prevent="removeFile(file)" class="absolute top-2 right-2 w-6 h-6 bg-errColor rounded-full p-1 hover:shadow-lg hover:opacity-100 opacity-80" tabindex="0" aria-label="Удалить">
                ...
                <div v-else class="flex h-full w-full px-2 py-1 rounded-lg border border-tertiary">
                  <a :href="file.url" :download="'file-' + index" class="hover:opacity-75 transition-all underline flex-grow p-2" :aria-label="`Скачать файл: ${file.name || 'Без названия'}`">Скачать файл {{ index + 1 }}</a>
                  <button @click.prevent="removeFile(file)" class="w-6 h-6 opacity-80 hover:opacity-100 mt-2 bg-errColor rounded-full p-1" tabindex="0" aria-label="Удалить">
                    <img src="/delete.svg" alt="Удалить" class="w-6 h-6" role="presentation">
                  </button>
                </div>
              </div>
          ...
          <div>
            ...
            <input type="file" multiple id="new-attachments" @change="handleFileUpload" ref="fileInput" class="my-file">
      ...
      <div v-else>
        <form @submit.prevent="submitAnswer" class="flex flex-col">
          <label for="answer-text" class="sr-only">Текст ответа</label>
          <textarea id="answer-text" v-model="answerText" class="my-input h-28 w-1/2" placeholder="Начните писать ответ..." :aria-label="answerText"></textarea>
